...
router.get('/types', async (req, res) => { //me trae todos los tipos de pokemones
    try{
        ...
        return res.status(500).json({msg:'error create types', e})
    }
});
...
router.post('/pokemons', async (req,res)=>{
    const {name, hp, attack, defense, speed, height, weight, img, types, createInDb} = req.body;
     if(!name) return res.status(404).json({msg:"name require"})
     try{
        if(name){
            const AllPokemones = await getAllPokemon(); //si ya existe el nombre que le paso
            const Poke= AllPokemones.find((e)=> e.name.toLowerCase() === name.toLowerCase()) // lo busco
            console.log(Poke)
           if(Poke) return res.status(401).send('existe')
        ...
        return res.status(500).json({msg:'error create pokemions',e })
     }
})
